...
void pci_scan(void) {
    ...
    for (uint16_t bus = 0; bus < 256 && device_count < PCI_MAX_DEVICES; bus++) {
        for (uint8_t slot = 0; slot < 32 && device_count < PCI_MAX_DEVICES; slot++) {
            ...
            uint32_t header_type_data = config_read((uint8_t)bus, slot, 0, 0x0C);
            uint8_t header_type = (uint8_t)(header_type_data >> 16);
            uint8_t num_functions = (header_type & 0x80) ? 8 : 1;

            for (uint8_t function = 0; function < num_functions && device_count < PCI_MAX_DEVICES; function++) {
                uint32_t func_id = config_read((uint8_t)bus, slot, function, 0);
                uint16_t func_vendor = (uint16_t)(func_id & 0xFFFF);
                if (func_vendor == 0xFFFF) continue;

                uint32_t class_data = config_read((uint8_t)bus, slot, function, 8);
